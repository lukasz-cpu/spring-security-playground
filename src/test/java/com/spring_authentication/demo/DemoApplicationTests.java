...
class DemoApplicationTests {
    ...
    void shouldReturn401ForHelloWithoutAuthentication() {
        WebClientResponseException exception = assertThrows(WebClientResponseException.class,
                () -> webClient().get().uri("/api/authn/hello?name=jim").retrieve().bodyToMono(String.class).block());
    ...
    void shouldReturnHelloWorldForHelloWithValidBasicAuth() {
        String response = webClient().get().uri("/api/authn/hello?name=jim")
                .header(HttpHeaders.AUTHORIZATION, basicAuth()).retrieve().bodyToMono(String.class).block();
    ...
    void shouldReturnHelloWorldForPublicEndpointWithoutAuthentication() {
        String response = webClient().get().uri("/api/anonymous/hello?name=jim").retrieve().bodyToMono(String.class)
                .block();
